function filtered_I = mean_shift_filter(I,sig_s,sig_r)
    [r, c] = size(I); % rows and columns
    BWby2 = ceil(3*sig_s)+1; % BW = 6*sig_s + 1
    e = 0.01; % when to stop the convergence process
    ...
    for x = 1:c
        for y = 1:r
            ...
            while 1
                ...
                Gs = exp(-1*((X-f(1)).^2+(Y-f(2)).^2)/(2*sig_s^2)); % spatial gaussian weights
                Gr = exp(-1*((LI-f(3)).^2)/(2*sig_r^2)); % range gaussian weights
                G = Gs.*Gr;
                Wp = sum(G,'all');
                fx = sum(G.*X,'all')/Wp;
                fy = sum(G.*Y,'all')/Wp;
